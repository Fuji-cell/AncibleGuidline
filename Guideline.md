...
|ディレクトリ名/ファイル名 |説明 |
|:--- |:--- |
|＜playbooks＞/ |プレイブックのホームディレクトリ（任意の名前） |
|├  site.yml |マスタープレイブック。ファイル名は「site.yml」とする。ロール定義用プレイブックを読み込む、必ず呼び出すプレイブック。|
|├  ansible.cfg |Ansibleのベース設定ファイル（Linuxで言うところのhttpd.confファイルのようなイメージ） |
|├  inventory |管理対象のノードを定義するインベントリファイル |
|├  log |ログ格納用ディレクトリ |
|├  <プレイブック名>.yml |ロール定義用プレイブック。必要なロールを指定し呼び出すプレイブック |
|│      　　　　：  | |
|├─host_vars/ |変数定義ファイル<host名>.ymlを配置するディレクトリ |
|│      <ノード名>.yml |ノード個別変数定義ファイル |
|│      　　　　：  | |
|├─group_vars/ |ノード共通で指定する変数定義ファイルの配置ディレクトリ |
|│      <グループ名>.yml |ノード共通変数定義ファイル |
|│      　　　　：  | |
|│ | |
|└─roles/ |ロール配置用ルートディレクトリ |
|&emsp;&emsp;├─<ロール名>/ |個別ロール用ディレクトリ |
|&emsp;&emsp;│  ├─vars/ |Role内で使用する変数を記載したファイルを配置するディレクトリ |
|&emsp;&emsp;│  │      main.yml |変数定義ファイル |
|&emsp;&emsp;│  ├─tasks/ |タスク処理を記載したファイルを配置するディレクトリ |
|&emsp;&emsp;│  │      main.yml |全タスク呼び出し定義ファイル |
|&emsp;&emsp;│  │      <タスク名>.yml |作業単位のタスクを定義したファイル |
|&emsp;&emsp;│  ├─handlers/ |ハンドラーを記載したファイルを配置するディレクトリ |
|&emsp;&emsp;│  │      main.yml |ハンドラー定義ファイル |
|&emsp;&emsp;│  ├─templates/ |テンプレートファイルを配置するディレクトリ |
|&emsp;&emsp;│  │      <テンプレート名>.j2 |テンプレート定義ファイル |
|&emsp;&emsp;│  │   　　　　　　　　　： | |
|&emsp;&emsp;│  ├─files/ |ファイル配置のルートディレクトリ |
|&emsp;&emsp;│  │      <ファイル名> |配布するファイルの配置ディレクトリ |
|&emsp;&emsp;│  │   　　　　　　　　　： | |
|&emsp;&emsp;│  ├─defaults/ |デフォルト値を記載したファイルを配置するディレクトリ |
|&emsp;&emsp;│  │      main.yml |ロール内での変数のデフォルト値を記載したファイル |
|&emsp;&emsp;│  ├─meta/ |ロールに対するメタ情報を配置するディレクトリ |
|&emsp;&emsp;│  │      main.yml |ロール内でのメタデータを記載したファイル |
